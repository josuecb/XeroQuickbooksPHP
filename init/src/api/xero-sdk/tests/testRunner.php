...
include_once $_SERVER['DOCUMENT_ROOT'] . '/quickbooks/init/src/api/JC/Helpers.php';
...
//        $response['oauth_session_handle']   =    $_SESSION['session_handle'];
        if(sizeof($response)>1){
            ...
            Helpers::insertUser($key, Helpers::XERO_CODE);
            $userId = Helpers::getUserId($key);
            try {
                Helpers::insertCredentials($userId, $response['oauth_token'], $response['oauth_token_secret'],Helpers::XERO_CODE);
            } catch (Exception $e) {
//            echo $e->getMessage();
                Helpers::updateCredentials($userId, $response['oauth_token'], $response['oauth_token_secret']);
            }
