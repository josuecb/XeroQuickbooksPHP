    ...
    if (isset($_REQUEST['invoice'])) {
        if (!isset($_REQUEST['method'])) {
            ...
            if ($XeroOAuth->response['code'] == 200) {
                ...
                echo "<br>There are " . count($invoices->Invoices[0]) . " invoices in this Xero organisation, the first one is: </br>";

                parseInvoices($invoices->Invoices[0]);
...
{
    $count = 1;
    foreach ($invoices as $itemss) {
        echo "$count<br><br>";
        foreach ($itemss as $key => $item) {
            echo "$key";
            if (is_object($item)) {
                foreach (xml2array($item) as $key2 => $value) {
                    if (sizeof($item) > 1) {
                        echo "<br>$key2 ";
                    }
                    if (is_array($value)) {
